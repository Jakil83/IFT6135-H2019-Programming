...
from modelsQ52k import RNN, GRU 
...
if args.model == 'RNN':
 ...
 #HERE: Load best model 
    checkpoint = torch.load('best_models/RNN_ADAM/best_params.pt', map_location=device)
    ...
    checkpoint = torch.load('best_models/GRU_SGD/best_params.pt', map_location=device)
    model.load_state_dict(checkpoint)
...
def normalize(x): 
    ...
    print(norm_data)
    #x = minVal + norm_data*(maxVal - minVal)
    return norm_data

def figure(x,y,title):
    plt.plot(x,y)
    plt.xlabel('Sequence steps')
    plt.ylabel('Norm of gradient loss w.r.t hidden states')
    
    plt.title(title, fontsize=14)
    
    plt.legend(fontsize=11)
    plt.savefig(args.save_dir+'/Q5.2.pdf',  bbox_inches='tight')
    plt.show()
...
def run_epoch(model, data, is_train=False, lr=1.0):
    ...
    for step, (x, y) in enumerate(ptb_iterator(data, model.batch_size, model.seq_len)):
        ...
        if args.model == 'TRANSFORMER':
            ...
            outputs, hidden, all_hiddens = model(inputs, hidden)
            #all_hiddens = model.all_hidden
            #all_hiddens = Variable(torch.stack([torch.stack(i) for i in all_hiddens]), requires_grad = True)
        ...
        if is_train:  # Only update parameters if training 
            ...
            for i in range(model.seq_len):
                h_grad.append(torch.autograd.grad(loss_per_step[-1], all_hiddens[i], retain_graph=True))
            
            h_grad = torch.stack([torch.stack(i) for i in h_grad])

            averaged_h_grad = torch.mean(h_grad, dim =-2)
            norm_h_grad = torch.mean(torch.norm(averaged_h_grad, p=2, dim=-1), dim=-1)
            

            print(norm_h_grad.size())
            print(norm_h_grad)
            
            figure(range(model.seq_len), normalize(norm_h_grad).numpy(), args.model)
            
            exit()
            #print(normalize(norm_h_grad))
