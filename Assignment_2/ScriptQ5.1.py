...
from modelsQ51 import RNN, GRU 
from modelsQ51 import make_model as TRANSFORMER
import pickle
...
if args.model == 'RNN':
    ...
    checkpoint = torch.load('best_models/TRANSFORMER_SGD/best_params.pt', map_location=device)
    model.load_state_dict(checkpoint)
...
def figure(x,y,title):
    ...
    plt.savefig(args.save_dir+'/Q5.1.pdf',  bbox_inches='tight')
    plt.show()
...
for epoch in range(num_epochs):
    ...
    print(val_loss)
    #Plot here#<--------------------
    figure(range(model.seq_len),val_loss.detach().numpy(),args.model)
    with open('TRANSFORMER.pickle', 'wb') as handle:
        pickle.dump(val_loss, handle, protocol=pickle.HIGHEST_PROTOCOL)
    #torch.save(val_loss, os.path.join(args.save_dir, 'RNNloss.pt'))
    exit()
